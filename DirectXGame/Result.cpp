...
void Result::Initialize() {
	...
	sprite_[10] =
	    Sprite::Create(textureSprite_[10], pos_[1], {1.0f, 1.0f, 1.0f, 1.0f}, {0.5f, 0.5f});
	...
	sprite_[12] = Sprite::Create(
	    textureSprite_[12], {655.0f, 325.0f}, {1.0f, 1.0f, 1.0f, 1.0f}, {0.5f, 0.5f});
...
void Result::Update() {
	if (gameinput_->CommandClear()) {
		timer_++;
	} else if (gameinput_->CommandClear() == false) {
		timer_ = 0;
	}
	Oncollision();
	/*ImGui::Begin("Result");
	ImGui::DragInt("counter", &counter_, 1);
	ImGui::End();*/
}
